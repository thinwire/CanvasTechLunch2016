 ...
 * Step 6: Optimize
 * 
 * Get rid of the active redraw loop; it's using a lot of
 * system resources while accomplishing nothing. We only
 * need to redraw when something in the system changes,
 * so that's when we call requestAnimationFrame to get a
 * clean slice of time to do our painting.
 */
...
class Spreadsheet {
    ...
    private init(): void {
        ...
        // Make scrollbars update the spreadsheet
        this._vscrollbar.onUpdate = this._hscrollbar.onUpdate = () => {
            this.update();
        };
        ...
        // Call redraw once to get image on screen
        this.update();
    }
