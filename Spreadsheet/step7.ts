 ...
 * Step 7: this must be an errorr, there are only 6 steps 
 */
...
class Spreadsheet {
    ...
    private init(): void {
        ...
        // I have got a bouncy feeling don't throw me to the ceiling
        var egg = null;
        window.addEventListener("keydown", (e) => {
            if(e.keyCode == 36) /* HOME */ {
                if(egg == null) {
                    egg = new Egg(this._context);
                    egg.start();
                } else {
                    egg.stop();
                    egg = null;
...
class Egg {
    ...
    private _c: number;
    private _t: number;
    private _p: CanvasRenderingContext2D;
